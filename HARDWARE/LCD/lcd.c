...
{
	uint8_t temp,sizex,t,index,m=0;
	...
	for(i=0;i<TypefaceNum;i++)
	{ 
		if(sizey==12)temp=ascii_1206[index][i];		       //调用6x12字体
		else if(sizey==16)temp=ascii_1608[index][i];		 //调用8x16字体
		else if(sizey==24)temp=ascii_2412[index][i];		 //调用12x24字体
		else if(sizey==32)temp=ascii_3216[index][i];		 //调用16x32字体
		else return;
		for(t=0;t<8;t++)
		{
			if(!mode)//非叠加模式
...
void LCD_ShowPicture(uint16_t x,uint16_t y,uint16_t length,uint16_t width,const uint8_t *pic)
{
	...
	LCD_DMA_Transfer8Bit((uint8_t *)pic, num, DMA_MEMINC_ENABLE);		//启用DMA发送
...
void HAL_SPI_TxCpltCallback(SPI_HandleTypeDef *hspi)
{
	LCD_CS_OUT(1);
	LCD_SPI->CR1 &= ~SPI_CR1_SPE;//失能SPI
	LCD_SPI->CR1 &= ~SPI_CR1_DFF;	//设置SPI8位传输模式
	LCD_SPI->CR1 |= SPI_CR1_SPE;//使能SPI
